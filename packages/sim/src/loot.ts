...
/**
 * ОДИН аффикс из сида — то, чем перековывает кузнец. GDD §6.3.
 *
 * Та же `rollAffix`, что у генерации, а не вторая её копия: разойдись
 * они, и перекованный аффикс отличался бы от выпавшего правилами,
 * которых нет ни в одном документе. Пул семейств слота, лестница тиров
 * и масштаб по ilvl приходят оттуда же.
 *
 * Кузнец МОЖЕТ сменить семейство, и это ставка, а не недосмотр:
 * перековка — точечная переделка одного аффикса, и если бы семейство
 * сохранялось, она сводилась бы к бесплатному подъёму тира.
 */
export function rollOneAffix(
  seed: string,
  ilvl: number,
  slot: EquipmentSlot,
  balance: LootBalance,
): ItemAffix {
  return rollAffix(rngFromSeed(seed), Math.max(1, Math.floor(ilvl)), slot, balance);
}
