...
export const fighterConfigSchema = z.object({
  ...
  /**
   * Значения аффиксов «Верности руки» СПИСКОМ, а не суммой.
   *
   * Суммой их сложить нельзя: бюджет берёт N сильнейших, и из суммы
   * их обратно не разложить. Ровно та же причина, по которой
   * процентные семейства приходят списками.
   *
   * Пусто у монстров и у всех, кто собран без снаряжения: `accuracy`
   * тогда остаётся базовой характеристикой и ничего не теряет.
   */
  accuracyAffixes: z.array(z.number()).default([]),
...
export const combatBalanceSchema = z.object({
  ...
  items: z.object({
    ...
    familyBudget: z.object({
      ...
      /**
       * «Верность руки» — ПЛОСКОЕ семейство под тем же бюджетом.
       *
       * Заведено замером, а не по аналогии: у шанса уклонения есть
       * потолок (§4.2), и точность сверх некоторого значения
       * не даёт НИЧЕГО. На носителе `theft` 34-го уровня один аффикс
       * T1 берёт 90.2% побед, второй добавляет 7.5 п.п., третий 0.6,
       * четвёртый ноль. Без бюджета третий и четвёртый молча пропадали
       * бы — игрок надел бы их и не понял, почему ничего не изменилось.
       */
      truehand: z.int().min(1),
    }),
  }),
