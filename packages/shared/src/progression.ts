...
function offerRoll(seed: string, level: number, step: number): number {
  ...
  /* ЛАВИНА ОБЯЗАТЕЛЬНА, и без неё оффер был сломан.
     FNV-1a заканчивается умножением, поэтому строки, различающиеся
     ПОСЛЕДНИМ символом, дают хеши, различающиеся примерно на простое
     множитель (0x01000193 ≈ 16.8 млн) — это 0.4% диапазона. Шаги 0, 1
     и 2 различаются как раз последним символом, значит три броска
     оффера были почти одинаковой долей.

     Следствие было видно замером, а не рассуждением: все три карты
     оффера ВСЕГДА оказывались соседними в порядке колоды — 40 троек
     из 40. Наклоны с краёв колоды предлагались в полтора раза реже
     тех, что в середине, и «фильтр по билду» упирался в порядок строк
     в json. Лавина размешивает низкие биты обратно в высокие. */
  hash ^= hash >>> 16;
