...
/**
 * Бросок оффера. Приём и его обоснование — в `seededRoll`; здесь только
 * состав ключа: сид игрока, уровень и номер карты в тройке.
 */
function offerRoll(seed: string, level: number, step: number): number {
  return seededRoll(`${seed}:draft:${level}:${step}`);
}
