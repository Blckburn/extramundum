 ...
 * без фляг меряется САМА настройка, с флягами — то, что увидит игрок,
 * который ими пользуется. Числа расходятся, и показывать одно значило бы
 * выбрать за человека, какой вопрос он задал.
 *
 * ФЛЯГИ ТЕПЕРЬ ПЛАТНЫЕ И ПО УМОЛЧАНИЮ ИХ НОЛЬ (M3d, экономика), поэтому
 * колонка «с флягами» стала оценкой СВЕРХУ: она отвечает на вопрос
 * «сколько дойдёт, если игрок купил полный запас», а не «сколько дойдёт
 * у среднего игрока». Порог по-прежнему сторожит колонку БЕЗ фляг —
 * ровно потому, что она мерит настройку, а не кошелёк.
 *
 * Берётся ВЕРХНИЙ ТИР и полный запас: нижняя граница у него та же
 * величина, что мерилась раньше, а выбор тира — решение игрока, и мерить
 * его политикой замера значило бы мерить не то.
 */
function simulateRun(player, zone, segment, seedTag, withPotions) {
  ...
  const flaskTiers = balance.economy.flasks.tiers;
  const flask = flaskTiers[flaskTiers.length - 1];
  const flaskLow = flask.restore[0];

  let hp = maxHp;
  let potions = withPotions ? balance.economy.flasks.maxCharges : 0;
  let cleared = 0;

  for (let fight = 0; fight < balance.raid.fightsPerRun; fight++) {
    /* Фляга пьётся МЕЖДУ боями, и политика здесь простейшая: ниже
       того, что она вернёт, — пить. Сложная политика мерила бы
       мастерство игрока, а не настройку.

       Берётся НИЖНЯЯ граница диапазона: замер обязан быть оценкой
       снизу, а не средним по удаче. Иначе доходимость показывала бы
       забеги, которых игрок не увидит.

       Побочный эффект верхнего тира в модель НЕ входит: он ложится
       статусом в бой, а бой здесь считает движок по своим правилам,
       и вводить его половинчато значило бы мерить не игру. */
    if (potions > 0 && hp < maxHp * (1 - flaskLow)) {
      hp = Math.min(maxHp, Math.round(hp + maxHp * flaskLow));
      potions--;
    }
...
/* Порог стоит на ПЕРВОМ решении и на замере БЕЗ зелий. Первое решение —
   ...
   Без фляг — потому что фляга это выбор игрока (и покупка), а порог обязан мерить
...
if (AS_JSON) {
  ...
  console.log('');
  console.log('ВТОРОЕ ЧИСЛО — ОЦЕНКА СВЕРХУ, а не «то же самое с зельями».');
  console.log('До M3d три заряда давались БЕСПЛАТНО и были у каждого; теперь фляга');
  console.log('покупается за золото, а по умолчанию зарядов НОЛЬ. Второе число');
  console.log('считает полный запас верхнего тира по НИЖНЕЙ границе его диапазона —');
  console.log('то есть потолок для того, кто заплатил. Типичный игрок ближе');
  console.log('к первому числу, и порог проверяется по нему.');
  console.log('');
  console.log('');
  ...
  console.log('Оба берутся без фляг: фляга — выбор игрока и покупка, а число обязано');
