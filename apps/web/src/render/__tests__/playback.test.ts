...
describe('бюджеты во время боя', () => {
  ...
  it('эффектам подаются ЦЕЛЫЕ часы: дробные стоят шестнадцать байт на кадр', () => {
    /**
     * Дробное число, переданное аргументом в вызов, который V8 не
     * встроил, приходится материализовать в куче — HeapNumber,
     * шестнадцать байт, каждый кадр. Замерено сменой одного аргумента:
     * `fx.update(this.clock)` даёт 10.1 байта на кадр, целое — ноль,
     * при одинаковой работе. `loop.update(dt)` и `particles.update(dt)`
     * от дробного аргумента не аллоцируют, в том числе на переменном
     * dt: дело не в «дробном аргументе вообще», а в этом вызове.
     *
     * ЗДЕСЬ ПРОВЕРЯЕТСЯ ФОРМА КОДА, А НЕ БАЙТЫ, и это та же причина,
     * что абзацем выше: `heapUsed` — величина на весь процесс. Байты
     * меряет scripts/render-budget.mjs отдельным процессом, и там
     * стоит диверсия: дробные часы в обвязке роняют бюджет (10.6
     * при пределе 0). Проигрыватель же в юнит-тест не поднять —
     * всплывающим числам нужен DOM, а окружение тестов node.
     */
    const player = code('../battle/player.ts');
    const start = player.indexOf('advance(dt: number)');
    expect(start, 'не нашли advance — тест смотрит не туда').toBeGreaterThan(0);
    const body = player.slice(start, player.indexOf('private refresh', start));

    // Часы эффектов округляются, и округляются именно из часов боя.
    expect(body).toMatch(/this\.effectsMs = Math\.trunc\(this\.clock\)/);

    // А дальше в кадре идут только они. Диверсия: вернуть сюда
    // `this.clock` — падает эта проверка, а следом и бюджет в CI.
    for (const call of ['this.numbers.collect', 'this.fx[0].update', 'this.fx[1].update']) {
      expect(body, `${call}: дробные часы — это аллокация в кадре`).toContain(
        `${call}(this.effectsMs)`,
      );
      expect(body, `${call}: дробные часы — это аллокация в кадре`).not.toContain(
        `${call}(this.clock)`,
      );
    }
  });
