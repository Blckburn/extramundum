...
export type BattleToShow = {
  ...
  /**
   * Бой ДОСМОТРЕН. Зовётся один раз, когда воспроизведение дошло
   * до конца — в том числе кнопкой «сразу итог».
   *
   * Нужен затем, что итог боя нельзя показывать раньше: экран рейда
   * до этой правки выкладывал панель наград и кнопку «продолжить»
   * ВМЕСТЕ с ареной, то есть строка «Отряд погиб» стояла над боем
   * с первого кадра. Единственная причина смотреть бой уничтожалась
   * одной кнопкой.
   */
...
export async function mountBattle(
  ...
  const refresh = (): void => {
    ...
    if (player.finished && summary.hidden) {
      // Сообщить наружу ДО показа собственной строки: экран рейда
      // раскрывает свою панель тем же событием, и порядок здесь
      // не важен, а вот однократность — да, её держит `summary.hidden`.
      battle.onFinished?.();
