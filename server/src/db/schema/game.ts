...
export const players = pgTable(
  'players',
  {
     ...
     * Сид драфта. GDD §5.2.
     *
     * Оффер трёх карт НЕ ХРАНИТСЯ: он выводится из этого сида и номера
     * уровня, поэтому сервер пересчитывает его и при показе, и при
     * применении выбора — и принимает только то, что сам же предложил.
     * Подделать выбор нечем: карты, которой не было в оффере, сервер
     * не найдёт.
     *
     * Сид на игрока, а не на уровень: тогда «что предложат» не зависит
     * от того, сколько раз игрок обновил экран, и одинаково у показа
     * и у применения.
     */
    draftSeed: text('draft_seed')
      .notNull()
      .default(sql`gen_random_uuid()::text`),

    /**
     * Сид кузнеца. НА СЕРВЕРЕ И ТОЛЬКО ЗДЕСЬ, как и сид драфта.
     *
     * Без него детерминизм попытки обернулся бы против игры: номер
     * предмета и счётчик попыток видны клиенту, `seededRoll` лежит
     * в `shared` и потому попадает в браузер, — то есть игрок посчитал
     * бы исход СЛЕДУЮЩЕЙ попытки заранее и жал бы только на удачные.
     * Риск из §6.3 исчез бы целиком, а вместе с ним и смысл угля.
     *
     * Отдельный от `draft_seed`: один сид на две системы означал бы,
     * что по известным офферам драфта восстанавливается поток кузнеца.
     */
    smithSeed: text('smith_seed')
      .notNull()
      .default(sql`gen_random_uuid()::text`),

    /**
     * Купленные вкладки стеша СВЕРХ стартовой. GDD §6.3.
     *
     * ЕДИНСТВЕННЫЙ СТОК, КОТОРЫЙ НЕ НАСЫЩАЕТСЯ ПО СМЫСЛУ: кузнец
     * упирается в улучшенный комплект, фляги — в полный запас,
     * а место под вещи нужно ровно столько, сколько вещей копится.
     * Ценами он всё же ограничен — их в балансе три, и это решение
     * человека, а не потолок механики.
     */
    stashTabs: integer('stash_tabs').notNull().default(0),
