  ...
  app.get(API_ROUTES.zones, async (c) => {
    ...
    const zones: ZoneCard[] = ZONES.map((zone) => {
      ...
      const segments = zone.segments.map((segment, index) => ({
        ...
        /* Множитель силы ЭТОГО участка. Показывается рядом с уровнями,
           потому что это вторая половина ответа на «насколько тут
           трудно»: уровень говорит, как глубоко, множитель — насколько
           само место тяжёлое. */
        power: segment.power,
